...
PLACE_ON_MAP targets a raster map, recognised by tile images addressed as `z/x/y` inside an overflow-hidden container; its tiles are left out of the page's media list. A vision model (`MAP_MODEL`, default `google/gemini-3.8-flash`, through the text helper's endpoint and key) receives the goal, page text, recent actions and a viewport screenshot, and returns exactly `place`, `lat` and `lng`, or a null place when the page asks for no location. The executor derives the Web Mercator projection from a loaded tile's URL and box, picks the world copy that can be clicked, and, when the place is covered or off screen, drags the map in slow steps (fast drags coast on) until the place lies in open map. It then clicks only a point whose hit test lands on the map itself, not on an overlay, marker or control. After a placement, the map's value reads `point placed: …` until another action runs, just as a filled field shows its value. When the helper declines, nothing is clicked, the decline is logged as a step that changed nothing, and the map is not offered again on that identical page.
